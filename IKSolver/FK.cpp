...
// main function that performs forward kinematics. Each joint has its local transformation relative to the parent joint. 
// globalTransform is the transform that converts a point from a joint's local space, to world
// localTransform is the transform that converts a point from a joint's local space, to parent joint's space.
// so then to get a local space point to parent space: xParent = localTransform * xLocal
// but note: globalTransform = parentGlobalTransform * localTransform, so the root's globalTransform = localTransform
...
{
    // first, compute the localTransform for each joint
    for (int i = 0; i < localTransforms.size(); i++)
    {
        double eulerRotation[9];
        euler2Rotation(eulerAngles[i], eulerRotation, rotateOrders[i]);

        double jointRotation[9];
        euler2Rotation(jointOrientationEulerAngles[i], jointRotation, XYZ); // from Piazza

        localTransforms[i] = RigidTransform4d((Mat3d(jointRotation) * Mat3d(eulerRotation)), translations[i]);
    }

    // then, recursively compute the globalTransforms, from the root to the leaves of the hierarchy.
    for (int i = 0; i < localTransforms.size(); i++)
    {
        if (jointParents[jointUpdateOrder[i]] >= 0) // just make sure this isnt the root
        {
