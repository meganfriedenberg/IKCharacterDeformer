...
void Skinning::applySkinning(const RigidTransform4d * jointSkinTransforms, double * newMeshVertexPositions) const
{
    ...
    for (int i = 0; i < numMeshVertices; i++)
    { // init to 0, will store values in cache
    ...
    for (int vtxID = 0; vtxID < numMeshVertices; vtxID++)
    {
        restPosition = Vec4d(restMeshVertexPositions[3 * vtxID + 0], restMeshVertexPositions[3 * vtxID + 1], restMeshVertexPositions[3 * vtxID + 2], 1.0f);
        for (int i = 0; i < numJointsInfluencingEachVertex; i++) // compute the new position for the vertices of joints
        {
            double currWeight = meshSkinningWeights[vtxID * numJointsInfluencingEachVertex + i]; // look up the weight
            newPosition = currWeight * jointSkinTransforms[meshSkinningJoints[vtxID * numJointsInfluencingEachVertex + i]] * restPosition; // formula

            newMeshVertexPositions[3 * vtxID + 0] += newPosition[0]; // MUST BE +=, EQUAL DOESN'T WORK, since a vertex is affected by multiple joints
            newMeshVertexPositions[3 * vtxID + 1] += newPosition[1];
            newMeshVertexPositions[3 * vtxID + 2] += newPosition[2];
        }
    }
